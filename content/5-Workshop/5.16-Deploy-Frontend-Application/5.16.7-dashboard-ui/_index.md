---
title : "Create the Dashboard Interface"
date : 2026-07-05
weight : 7
chapter : false
pre : " <b> 5.16.7 </b> "
...
export default function Dashboard({ email, onSignOut }: DashboardProps) {
  ...
  async function downloadReport(report: DataItem) {
    ...
    if (!reportDate) {
      setError("Report metadata has no valid date.");
      return;
    }
    ...
    try {
      ...
      if (!result.downloadUrl) {
        throw new Error(result.message || "API did not return a downloadUrl.");
      }
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="mx-auto max-w-7xl space-y-8 px-6 py-8">
        <section className="flex flex-col justify-between gap-4 rounded-2xl bg-emerald-700 p-6 text-white shadow-lg md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-2 text-sm text-emerald-50">
              Track power usage, waste alerts, and daily reports.
            </p>
          </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <SummaryCard title="Rooms" value={rooms.length} description="Configured rooms" />
          <SummaryCard title="Telemetry" value={telemetry.length} description="Last 20 samples" />
          <SummaryCard title="Alerts" value={alerts.length} description="Latest alerts" />
          <SummaryCard title="Reports" value={reports.length} description="Generated reports" />
        </section>
          ...
          <>
            <section className="rounded-2xl bg-white shadow-sm">
              <div className="border-b border-slate-200 px-6 py-4">
                <h2 className="text-lg font-bold">Latest Telemetry</h2>
              </div>
            ...
            <section className="rounded-2xl bg-white shadow-sm">
              <div className="border-b border-slate-200 px-6 py-4">
                <h2 className="text-lg font-bold">Waste Alerts</h2>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {alerts.map((item, index) => (
                      <tr key={displayValue(item.SK, String(index))}>
                        ...
                        <td className="px-6 py-3">
                          {displayValue(
                            item.message ?? item.reason ?? "Energy waste detected"
                          )}
                        </td>
                      </tr>
                    ))}
            ...
            <section className="rounded-2xl bg-white shadow-sm">
              <div className="border-b border-slate-200 px-6 py-4">
                <h2 className="text-lg font-bold">Energy Reports</h2>
              </div>
...
![Dashboard.tsx in the project structure](/images/5-Workshop/5.16/pic2.png)
