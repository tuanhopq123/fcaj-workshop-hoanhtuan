...
Mã nguồn của hàm Lambda sẽ thực hiện các tác vụ sau:
- Nhận `reportDate` khi test thủ công.
- Nếu không có `reportDate`, chọn ngày hôm trước theo UTC+7.
- Query `TELEMETRY#YYYY-MM-DD`.
- Query `ALERT#YYYY-MM-DD`.
- Tổng hợp công suất và điện năng.
- Tạo file báo cáo JSON.
- Ghi file vào S3.
- Ghi metadata vào DynamoDB.

**Quy ước tên file và đường dẫn lưu trữ:**
- Tên file báo cáo: `energy-report-<roomId>-<YYYY-MM-DD>.json` (Ví dụ: `energy-report-lab-01-2026-07-03.json`)
- Object key (đường dẫn trên S3): `reports/2026-07-03/energy-report-lab-01-2026-07-03.json`

#### Bước 1: Mở Code source

1. Quay lại trang cấu hình Lambda `energy-report-generator`.
2. Chọn tab: **Code**.
3. Mở file: `lambda_function.py`.
4. Xóa toàn bộ mã mẫu mặc định.

#### Bước 2: Dán mã nguồn

Sao chép và dán toàn bộ đoạn code Python sau vào trình soạn thảo:
...
2. Chờ hệ thống hiển thị thông báo: Successfully updated the function energy-report-generator.

Lưu ý: Không bấm "Test" trước khi bấm "Deploy".

![Triển khai mã nguồn](/images/5-Workshop/5.15-lambda-report/5.15.5-deploy-code.png)