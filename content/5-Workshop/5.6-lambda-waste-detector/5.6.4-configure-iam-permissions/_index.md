---
title : "Configure IAM Permissions for Lambda"
date : 2026-07-01
weight : 4
chapter : false
pre : " <b> 5.6.4 </b> "
...
1. **Open the execution role**
   * On the Lambda page, go to **Configuration → Permissions**.

![Lambda Permissions - Execution role](/images/5-Workshop/5.6-lambda/pic5.png)

2. **Open the IAM role**
   * Under **Execution role**, click the role name.
   * The role name usually looks like: `energy-waste-detector-role-pmsoahv7`.
   * This opens the IAM Role page.

3. **Create an inline policy**
   * In the IAM Role, select **Add permissions → Create inline policy**.
   * Select the **JSON** tab.
...
![Create inline policy - JSON editor](/images/5-Workshop/5.6-lambda/pic6.png)
   ...
   * Click **Next**.
   * Set the policy name to: `energy-waste-detector-inline-policy`.
   * Click **Create policy**.

![IAM Role - inline policy created](/images/5-Workshop/5.6-lambda/pic7.png)

4. **Paste the Lambda function code**
   * Go back to the `energy-waste-detector` Lambda tab.
   * Go to the **Code** tab.
   * Open `lambda_function.py`.
   * Delete the existing code and paste this code:
...
![Lambda code pasted and ready to deploy](/images/5-Workshop/5.6-lambda/pic8.png)
